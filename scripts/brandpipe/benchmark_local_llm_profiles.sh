...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      cat <<'EOF'
Usage: scripts/brandpipe/benchmark_local_llm_profiles.sh [options]
...
Options:
  --out-dir <path>                Output directory (default: /tmp/brandpipe_local_llm_bench_<timestamp>)
...
if [[ -z "$OUT_DIR" ]]; then
  OUT_DIR="/tmp/brandpipe_local_llm_bench_$(date +%Y%m%d_%H%M%S)"
fi
mkdir -p "$OUT_DIR"
...
if ! zsh "$ROOT_DIR/scripts/brandpipe/preflight_llm.sh" \
  --check-lmstudio \
...
run_probe() {
  ...
  local cmd=(
    python3
    "$ROOT_DIR/scripts/brandpipe/local_llm_warm_cache_probe.py"
    --provider="$provider"
    --base-url="$base_url"
    --model="$model"
    --runs="$RUNS"
